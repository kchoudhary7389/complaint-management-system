  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post(
        `${import.meta.env.VITE_BASE_URL}/admin/update-complaint/${id}`,
        { status, remark, complaintId: id },
    ...
    const fetchComplaintDetails = async () => {
      ...
      try {
        const res = await axios.get(
          `${import.meta.env.VITE_BASE_URL}/admin/complaint-details/${id}`,
          { headers: { Authorization: `Bearer ${token}` } }
        );
        if (res.status === 200) {
    ...
    <div className="font-['ubuntu']">
      ...
      <div className="flex flex-col sm:flex-row gap-5">
        ...
        <div className="p-5 w-full">
          <h3 className="sm:text-4xl text-2xl sm:text-left text-center uppercase">complaint details</h3>
          {message && (
          ...
          <table className="w-full sm:table hidden mt-10 border border-gray-300 shadow-lg rounded-lg overflow-hidden">
            {complaintDetails.map((detail, index) => (
              <tbody key={index} className="bg-white divide-y divide-gray-300">
                {/* Row 1 */}
                <tr className=" text-gray-800 ">
                  <th className="px-6 border border-gray-600 py-3 text-left font-semibold">
                    Full Name
                  </th>
                  <td className="px-6 border border-gray-600 py-4 text-gray-700">
                ...
                <tr>
                  ...
                  <td className="px-6 border border-gray-600 py-4 text-gray-700">
                    <td className="px-6  text-center text-white">
                      <span
                ...
                <tr>
                  <td className="p-6">
                    <button
                      ...
                      className={`bg-green-600 p-1 inline-block rounded text-white ${
                        detail.status === "Closed"
                          ? "bg-gray-600 hidden cursor-not-allowed"
                  ...
                  <td className="p-6">
                    <Link
                      to={`/admin/user-details/${detail.userId._id}`}
                      className="bg-green-600 cursor-pointer p-1 inline-block rounded text-white"
                    >
                      User Details
                    </Link>
                  </td>
